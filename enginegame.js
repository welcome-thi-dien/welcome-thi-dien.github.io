function engineGame(options) {
  ...
  var game = new Chess();
  var board;
  var BoardCtor = window.ChessBoard || window.Chessboard;
  var boardId = options.boardId || "board";
  var onUpdate = options.onUpdate;
  var onSelectSquare = options.onSelectSquare;
  var onClearHighlights = options.onClearHighlights;
  var pieceTheme = options.pieceTheme;
  var promoteId = options.promoteId || "promote";
  var getPromotion = options.getPromotion;
  ...
  var evaler =
    typeof STOCKFISH === "function"
      ...
      : new Worker(options.stockfishjs || "src/stockfish.js");
  var engineStatus = {};
  var thinkDelayMs = options.thinkDelayMs || 0;
  var thinkTimeoutID = null;
  var lastFen = null;
  ...
  var evaluation_el = document.getElementById(options.evaluationId || "evaluation");
  ...
  function displayStatus() {
    ...
    if (!engineStatus.engineLoaded) {
      status = "Đang tải máy đánh...";
    } else if (!engineStatus.engineReady) {
      status = "Máy đang chuẩn bị...";
    } else if (engineStatus.search) {
      status = "Máy đang suy nghĩ...";
    } else {
      status = "Máy đã sẵn sàng.";
    }
  ...
  function syncBoard(lastMove) {
    ...
    if (lastMove && !lastMove.promotion) {
      board.move(lastMove.from + "-" + lastMove.to);
    } else {
      var fen = game.fen();
      if (fen !== lastFen) {
        board.position(fen);
  ...
  function prepareMove(lastMove) {
    ...
    if (!game.game_over()) {
      if (turn != playerColor) {
        ...
        if (time && time.wtime) {
          thinkTimeoutID = setTimeout(function () {
            uciCmd(
              "go " +
  ...
  engine.onmessage = function (event) {
    ...
    if (line == "uciok") {
      ...
      if (match) {
        ...
        var moveObj = game.move({ from: match[1], to: match[2], promotion: match[3] });
        prepareMove(moveObj);
        uciCmd("eval", evaler);
        if (evaluation_el) {
          evaluation_el.textContent = "";
  ...
  function readPromotion() {
    if (typeof getPromotion === "function") {
      return getPromotion();
    }
    var promoteEl = document.getElementById(promoteId);
    if (promoteEl && promoteEl.value) {
      return promoteEl.value;
    }
    return "q";
  }
  ...
  var onDrop = function (source, target) {
    ...
    var move = game.move({
      from: source,
      to: target,
      promotion: readPromotion(),
    });

    // illegal move
    if (move === null) return "snapback";

    if (typeof onClearHighlights === "function") {
      onClearHighlights();
    }
    prepareMove(move);
  };
  ...
  if (typeof options.moveSpeed !== "undefined") cfg.moveSpeed = options.moveSpeed;
  if (typeof options.snapbackSpeed !== "undefined") cfg.snapbackSpeed = options.snapbackSpeed;
  if (typeof options.snapSpeed !== "undefined") cfg.snapSpeed = options.snapSpeed;
  if (typeof options.appearSpeed !== "undefined") cfg.appearSpeed = options.appearSpeed;
  if (typeof options.trashSpeed !== "undefined") cfg.trashSpeed = options.trashSpeed;
